...
## 概要
**Origin-Twitter-Neo**は、[monefiera](https://github.com/monefiera)さんのOrigin-Twitterをベースに、参照元を更新して自動ビルドするようにした自分用ビルドリポジトリです。
...
- **ログイン不具合の解決策 (2026/01/05更新)**
  パッチの影響でログインできない不具合がありましたが、解決策が判明しました。[こちらの手順](https://github.com/crimera/piko/issues/714#issuecomment-3706542446)を実施することでログイン可能です。
- **署名の変更について (2025/12/27)**
  署名がされていない不具合を修正しました。これに伴い署名キーが変更されたため、以前のバージョンからは上書きアップデートができない場合があります。その際は一度アンインストールしてから再インストールしてください。これ以降、署名は変更しません。
- **Blueskyでのご紹介について**
  [もねふぃえ](https://github.com/monefiera)さんがご自身のBlueskyでこのリポジトリを紹介してくださっていました。勝手にフォークして作成したにも関わらず、ありがとうございます！🙇🏻
